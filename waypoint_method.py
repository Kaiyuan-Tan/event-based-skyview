...
def main():
    ...
    argparser.add_argument(
        ...
        default=20.0,
        type=float,
        help='Camera height above the ground')
    argparser.add_argument(
        '-pitch', '--pitch',
        default=-30.0,
        type=float,
        help='Camera pitch to the ground')
    argparser.add_argument(
        '-speed', '--speed',
        default=20.0,
        type=float,
        help='Camera moving speed in m/s')
    ...
    try:
        ...
        for n, transform in enumerate(spawn_points):
            ...
            if hero:
                blueprint.set_attribute('role_name', 'herspeedo')
                hero = False
            else:
        ...
        # spawn camera & setting parameters
        height = args.height
        pitch = args.pitch
        camera_speed = args.speed * 0.0333
        ...
        camera_init_trans = carla.Transform(carla.Location(z=height), carla.Rotation(pitch=pitch, yaw=0))
        ...
        while True:
            ...
            #spectator movement
            spectator.set_transform(current_wp.transform)
            next_wps = current_wp.next(camera_speed)
